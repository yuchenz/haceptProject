...
def evaluate(goldSubaFile, alignedSntFrameList, softEval=False, analysis=False):
	...
	for i, sntFrame in enumerate(alignedSntFrameList):
		if sntFrame == None:
			...
			autoTuples = set([suba[:4] for frame in sntFrame.frameList for suba in frame.subtreeAlignment_waMatrixPos])
			softAutoTuples = set([suba[:4] for frame in sntFrame.frameList for suba in frame.allPossibleSubtreeAlignment()])
			softAutoFrames = set([tuple([suba[:4] for suba in frame.allPossibleSubtreeAlignment()]) for frame in sntFrame.frameList]) 
